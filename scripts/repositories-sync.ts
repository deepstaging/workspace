...
async function main() {
  ...
  console.log(`Repositories directory: ${repositoriesDir}`);
  ...
  const repos = await scanRepositories(repositoriesDir);
      ...
      switch (strategy) {
        case 'ai':
          if (!(await checkCopilotAvailable())) {
            ...
            const updatedRepos = await scanRepositories(repositoriesDir);
            ...
            await pushRepositories(selected, updatedRepos, repositoriesDir, githubOrg, existingRepos);
            return;
          }
          break;
  ...
  await pushRepositories(selected, repos, repositoriesDir, githubOrg, existingRepos);
}
