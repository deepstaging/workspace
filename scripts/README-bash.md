...
```bash
npm run publish -- deepstaging [--clear] [--skip-build] [--version-suffix dev.abc123]
...
- **Automatic version suffixing** to prevent IDE cache issues
- Progress indicators and summary
- Optional feed clearing

> **Note**: Each publish automatically appends a unique timestamp suffix (e.g., `1.0.0-dev.abc123`) to prevent NuGet cache conflicts. This eliminates the need to clear all caches and avoids IDE project reload issues.
